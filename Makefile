...
build:
	...
	@RUST_TARGET_PATH=$(shell pwd) xargo rustc --release --target i686-unknown-plantos -Z build-std -- --emit obj=target/i686-unknown-plantos/release/$(PROGRAM_NAME).o
	$(LINK) target/i686-unknown-plantos/release/$(PROGRAM_NAME).o $(BASIC_LIB_C) -o target/i686-unknown-plantos/release/$(PROGRAM_NAME).bin
	mcopy -i ./img/disk.img target/i686-unknown-plantos/release/$(PROGRAM_NAME).bin ::
run:
